...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        //by lg
        // 不是gif，就马上开始下载
        if ( YES /*![_photo.url.absoluteString hasSuffix:@"gif"]*/)
        {
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        _imageView.frame = _photo.srcImageFrame;

            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationCurveEaseOut animations:^{
                _imageView.frame = imageFrame;

                if([self.photoViewDelegate  respondsToSelector:@selector(view)])
                {
                    UIView * view= (UIView *)[(UIViewController*)self.photoViewDelegate view];
